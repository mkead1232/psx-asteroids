...
void display() {
    ...
    // Temporarily offset the draw environment for screen shake
    DRAWENV shaken_env = draw[db]; // Make a copy
    ...
    PutDrawEnv(&shaken_env); // Use the shaken copy only this frame
    PutDrawEnv(&draw[db]);
